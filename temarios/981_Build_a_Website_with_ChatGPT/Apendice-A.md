...
Una vez que hayas copiado el código generado por ChatGPT en el portapapeles de tu computadora, el siguiente (y último) paso es pegar el código en un área de juegos de codificación web en línea. Un área de juegos es un sitio web que acepta código HTML, CSS y, a menudo, JavaScript, y luego muestra cómo se mostrará ese código en el navegador web. Al usar un área de juegos para probar tu código, evitas la molestia de guardar el código en un archivo y luego implementar el archivo en un servidor web, lo cual es demasiado trabajo cuando solo quieres ver si una sección o un componente de la página funciona correctamente.

A continuación se muestran algunos espacios de programación web que puedes consultar:
...
Todos estos parques infantiles funcionan más o menos de la misma manera:

Copia tu código ChatGPT.

Navega hasta el patio de juegos.

Haga clic dentro del editor que corresponde al tipo de código que está probando (por ejemplo, utilice el editor HTML para el código HTML).

Haga clic en el botón que ejecuta el código (o, en el caso de CodePen, simplemente espere unos segundos).

NOTA: si ChatGPT genera código HTML que también incluye código CSS (entre las etiquetas <style>y </style>), es mejor copiar el CSS por separado (ignorando las etiquetas <style>y </style>) y pegarlo en el editor CSS del área de juegos. De manera similar, si ChatGPT genera código HTML que también incluye código JavaScript (entre las etiquetas <script>y </script>), copie el código JavaScript por separado (omita las etiquetas <script>y </script>) y péguelo en el editor JavaScript. Para evitar el trabajo adicional de extraer CSS y JavaScript del código HTML generado, incluya en su mensaje que desea que ChatGPT muestre el HTML, CSS y JavaScript por separado. ChatGPT debe colocar el código HTML en una ventana HTML, el código CSS en una ventana CSS separada y el código JavaScript en una ventana JavaScript separada.

Mientras trabajas con ChatGPT, es una buena idea mantener tu área de juegos favorita abierta en su propia pestaña del navegador. De esa manera, tan pronto como ChatGPT genere un nuevo código para ti, puedes copiar y pegar el código inmediatamente en la área de juegos para comprobar que funciona.
...
Una página web sencilla puede utilizar un único archivo HTML que incluya código CSS y JavaScript. El código CSS va entre las etiquetas <style>y en una sección de encabezado especial de la página, y el código JavaScript va entre las etiquetas y , normalmente cerca de la parte inferior del archivo HTML. A continuación, se muestra el diseño general de un archivo HTML de este tipo:</style><script></script>
...
CSS —Pegue el código debajo de la <style>etiqueta pero encima de la </style>etiqueta.

HTML: pegue el código debajo de la <body>etiqueta pero encima de <script>ella. Los distintos componentes de la página deben aparecer en el archivo en el mismo orden en que desea que aparezcan en la página. Le daré instrucciones de ubicación más específicas en los capítulos en los que se generan los componentes de la página.

JavaScript: pegue el código debajo de la <script>etiqueta pero encima de la </script>etiqueta.
...
<html>
<cabeza>
    <link href="styles.css" rel="hoja de estilo">   ①
</cabeza>
<cuerpo>
    Las etiquetas HTML y el texto van aquí 
...
CSS: entre la <head>etiqueta y la </head>etiqueta, agregue una <link>etiqueta y establezca el hrefatributo igual al nombre del archivo CSS rodeado de comillas.

JavaScript : justo encima de la </body>etiqueta, agregue una <script>etiqueta con su srcatributo establecido en el nombre del archivo JavaScript rodeado de comillas y luego agregue una </script>etiqueta.

Para reutilizar un archivo CSS o JavaScript en varios archivos HTML, incluya la misma <link>etiqueta o <script>rótulo en esos archivos.
